...
local pathBlockIgnoreItems = { 2471 }

local function pathHasItem(path, startPos, itemIds)
    local checkPos = {x=startPos.x, y=startPos.y, z=startPos.z}
    for _, dir in ipairs(path) do
        local d = modPos(dir)
        checkPos = {x=checkPos.x + d[1], y=checkPos.y + d[2], z=checkPos.z}
        local tile = g_map.getTile(checkPos)
        if tile then
            for _, item in ipairs(tile:getItems()) do
                if table.find(itemIds, item:getId()) then return true end
            end
        end
    end
    return false
...
CaveBot.registerAction("goto", "green", function(value, retries, prev)
  ...
  if not CaveBot.Config.get("ignoreFields") and not pathHasItem(path, playerPos, pathBlockIgnoreItems) and CaveBot.walkTo(pos, 40) then
    return "retry"
  end
