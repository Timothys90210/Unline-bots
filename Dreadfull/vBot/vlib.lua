...
onTalk(function(name, level, mode, text, channelId, pos)
    ...
    for _, v in pairs(Spells) do
        if v.name and v.name:lower() == text then
            local words = v.words and v.words:lower()
            if words and SpellCastTable[words] then
                SpellCastTable[words].t = now
            end
            break
        end
    end
...
function canCast(spell, ignoreRL, ignoreCd)
    ...
    if SpellCastTable[spell] then
        if (now - SpellCastTable[spell].t > SpellCastTable[spell].d or ignoreCd) and
           (ignoreCd or not getSpellCoolDown(spell)) then
            return true
        else
            return false
        end
    ...
    if spellData then
        if (ignoreCd or not getSpellCoolDown(spell)) and
            (ignoreRL or level() >= spellData.level and mana() >= spellData.mana) then
            return true
        else
            return false
        end
...
function getBackpackByName(name)
    for i, container in pairs(g_game.getContainers()) do
        if container:getName():lower() == name:lower() then
            return container
        end
    end
    return nil
...
function moveItemByIdFromBackpackName(backpackName, itemId, slot)
    local container = getBackpackByName(backpackName)
    if not container then
        print("Backpack named '" .. backpackName .. "' not found!")
        return
    end
    ...
    for _, item in ipairs(items) do
        if item:getId() == itemId then
            ...
            local slotPosition = {x = 65535, y = slot, z = 0} -- Correct inventory slot position

            g_game.move(item, slotPosition, item:getCount())
            print("Moved item:", item:getId(), "to slot", slot)
            return
        end
    end

    print("Item ID " .. itemId .. " not found in '" .. backpackName .. "'.")
end
