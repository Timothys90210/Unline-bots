...
local furnitureIgnore = { 2986, 7527 }
...
CaveBot.registerAction("move", "green", function(value, retries, prev)
  -- Syntax: move:west,5 or move:north,3
  local match = regexMatch(value, "\\s*([a-zA-Z]+)\\s*,\\s*([0-9]+)")
  if not match[1] then
    warn("Invalid cavebot move value. Use: move:west,5 — got: " .. value)
    return false
  end

  local dir = string.lower(match[1][2])
  local steps = tonumber(match[1][3]) or 1
  ...
  local d = dirs[dir]
  if not d then
    warn("Invalid direction in move action: " .. dir)
    return false
  end
  ...
  local target = {
    x = playerPos.x + (d.x * steps),
    y = playerPos.y + (d.y * steps),
    z = playerPos.z
  }

  local gotoValue = string.format("%d,%d,%d", target.x, target.y, target.z)

  -- Try to find the goto action in any registry
  local gotoAction =
    (CaveBot.ActionsList and CaveBot.ActionsList["goto"]) or
    (CaveBot.actions and CaveBot.actions["goto"]) or
    (CaveBot.registeredActions and CaveBot.registeredActions["goto"])

  if gotoAction and gotoAction.callback then
    -- Found the registered goto action
    return gotoAction.callback(gotoValue, retries or 0, prev or nil)
  end

  --------------------------------------------------------------------------
  -- If not found, fallback to minimal inline goto-like logic
  --------------------------------------------------------------------------
  local maxDist = storage.extras.gotoMaxDistance or 40
  local playerPosNow = player:getPosition()
  if not playerPosNow then return false end

  if playerPosNow.z ~= target.z then
    noPath = noPath + 1
    pathfinder()
    return false
  end

  local path = findPath(playerPosNow, target, maxDist, {
  ...
  if CaveBot.walkTo(target, maxDist, { ignoreNonPathable = true, allowUnseen = true }) then
    return true
  end

  local current = player:getPosition()
  if current.x == target.x and current.y == target.y and current.z == target.z then
    return true
  end
