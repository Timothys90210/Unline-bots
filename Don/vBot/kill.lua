...
Panel
  ...
  Label
    ...
    font: verdana-9px-bold
      ...
      if lbls[k] then
        lbls[k].name:setText(string.sub(k, 1, 23) .. ':')
        lbls[k].count:setText(v)
      end
    end
  end
end
  ...
  for x = 1, #reg do
    ...
    if mobName then
      if killList[mobName] then
		if player:getOutfit().type == 957 then
			killList[mobName] = killList[mobName] + 2
		else
...
onTalk(function(name, level, mode, text, channelId, pos)
  if channelId == 12 then checkKill(mode, text) end
end)
