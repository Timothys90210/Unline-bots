...
-- Initialize storage.caveBot if it doesn't exist
...
CaveBot.Extensions.SupplyCheck.setup = function()
  CaveBot.registerAction(
    ...
    function(value)
      ...
      -- DEBUG OUTPUT
      modules.game_textmessage.displayGameMessage("DEBUG: Cap=" .. freecap() .. " Threshold=" .. tostring(supplyInfo.capacity.value) .. " Enabled=" .. tostring(supplyInfo.capacity.enabled))
      if type(supplyData) == "table" then
        modules.game_textmessage.displayGameMessage("DEBUG: Item check FAILED - ID:" .. supplyData.id .. " Amount:" .. supplyData.amount)
      else
        modules.game_textmessage.displayGameMessage("DEBUG: Item check PASSED")
      end

      storage.caveBot = storage.caveBot or {}

      -- Debug logging to file (DISABLED - io library not available in Unline)
      -- local logFile = io.open("C:/Users/Tim/AppData/Roaming/Unline/unline/bot/Testing/supply_check_debug.txt", "a")
      -- if logFile then
      --   logFile:write(os.date("%Y-%m-%d %H:%M:%S") .. " - Supply Check\n")
      --   logFile:write("  Free Cap: " .. tostring(freecap()) .. "\n")
      --   logFile:write("  Cap Check Enabled: " .. tostring(supplyInfo.capacity.enabled) .. ", Threshold: " .. tostring(supplyInfo.capacity.value) .. "\n")
      --   logFile:write("  Stamina Enabled: " .. tostring(supplyInfo.stamina.enabled) .. "\n")
      --   logFile:write("  Supply Data Type: " .. type(supplyData) .. "\n")
      --   if type(supplyData) == "table" then
      --     logFile:write("  Item ID: " .. tostring(supplyData.id) .. ", Amount: " .. tostring(supplyData.amount) .. "\n")
      --   end
      --   logFile:close()
      -- end
