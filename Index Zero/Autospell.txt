...
local spells = {
    ...
    EK = {
        ...
        { name="Executioners Throw", incantation="exori amp kor", mana=225, level=300, cooldown=1000, range=4 }
    },
    -- Paladin Spells
    RP = {
        ...
        { name="Sharpshooter", incantation="utito tempo san", mana=450, level=60, cooldown=7000, range=7 },
...
storage.autoSpellStopOnTrainer = storage.autoSpellStopOnTrainer or false

-- ### UI-BUTTONS ###
UI.Separator()

local autoSpellMSButton = macro(1000, "AutoSpell MS", function() end)
autoSpellMSButton.setOn(storage.MS_enabled or false)

local autoSpellEDButton = macro(1000, "AutoSpell ED", function() end)
autoSpellEDButton.setOn(storage.ED_enabled or false)

local autoSpellEKButton = macro(1000, "AutoSpell EK", function() end)
autoSpellEKButton.setOn(storage.EK_enabled or false)

local autoSpellRPButton = macro(1000, "AutoSpell RP", function() end)
autoSpellRPButton.setOn(storage.RP_enabled or false)

local autoSpellNinjaButton = macro(1000, "AutoSpell Ninja", function() end)
...
UI.Separator()
local stopOnTrainerButton = macro(20, "Stop near Trainers/Dummy", function() end)
stopOnTrainerButton.setOn(storage.autoSpellStopOnTrainer or false)
UI.Separator()
...
local function updateButtonStates()
    ...
    storage.autoSpellStopOnTrainer = stopOnTrainerButton:isOn()
end
...
local function countMonsters(range)
    ...
    for _, c in ipairs(creatures) do
        if c:isMonster() and (g_game.getClientVersion() < 960 or c:getType() < 3) and c:getName():lower() ~= ignoreName then
            count = count + 1
        end
    end
    return count
...
local function hasItemNearby(itemId, range)
    ...
    for _, tile in ipairs(g_map.getTiles(posz())) do
        local dist = distanceFromPlayer(tile:getPosition())
...
macro(500, function()
    ...
    if storage.autoSpellStopOnTrainer and (hasTrainerNearby(10) or hasItemNearby(29243, 10)) then return end
