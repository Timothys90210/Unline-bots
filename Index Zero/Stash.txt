...
-- Pending operations persistence (for crash recovery)
storage.pendingWithdraws = storage.pendingWithdraws or nil
storage.pendingDeposits = storage.pendingDeposits or nil
storage.lastOperationType = storage.lastOperationType or nil  -- "withdraw" or "deposit"
...
local stashWindowId = findWindowByTitle(g_ui.getRootWidget(), "Supply Stash")
...
local function V(callback)
  ...
  for _, btn in ipairs(children) do
    if btn:getId() == "stashButton" then
      btn:onClick()
      pollForWindow(stashWindowId, 2000, callback)
      return
    end
  end
  if callback then callback(nil) end
...
local function W(callback, show)
  local stashWindow = g_ui.getRootWidget():getChildById(stashWindowId)
  if stashWindow then
...
local function a8_wrapper(m, doneCb)
  ...
  storage.pendingDeposits = remaining
  storage.lastOperationType = "deposit"
  ...
  modules.corelib.periodicalEvent(function() end, function()
    ...
    if not v then
      ...
      storage.pendingDeposits = nil
      storage.lastOperationType = nil
      doneCb()
      return false
    end
    ...
    a2_with_confirm(tonumber(v), need, function(moved)
      ...
      -- Update persisted state after each action
      storage.pendingDeposits = remaining
...
local function ad_wrapper(m, doneCb)
  ...
  -- Persist state for crash recovery
  storage.pendingWithdraws = remaining
  storage.lastOperationType = "withdraw"
...
-------------------------------------------------------
-- 11. Crash Recovery - Resume pending operations
-------------------------------------------------------
local function resumePendingOperations()
  -- Wait for stash snapshot to be ready
  if not T or table.empty(T) then
    schedule(1000, resumePendingOperations)
    return
  end
  ...
  -- Check for pending withdraws
  if storage.lastOperationType == "withdraw" and storage.pendingWithdraws then
    local pending = storage.pendingWithdraws
    if not table.empty(pending) then
      print("[Stash] Resuming pending withdraws after restart...")
      ad_wrapper(pending, function()
        Z()
        print("[Stash] Pending withdraws completed.")
      end)
      return
    end
  end
  ...
  if storage.lastOperationType == "deposit" and storage.pendingDeposits then
    local pending = storage.pendingDeposits
    if not table.empty(pending) then
      print("[Stash] Resuming pending deposits after restart...")
      a8_wrapper(pending, function()
        Z()
        print("[Stash] Pending deposits completed.")
      end)
      return
    end
  end
...
-- Delay resume to allow client to fully initialize
schedule(3000, resumePendingOperations)

-- Utility: Clear all pending operations (call if stuck)
function clearPendingStashOperations()
  storage.pendingWithdraws = nil
  storage.pendingDeposits = nil
  storage.lastOperationType = nil
  a0 = false
  a1 = false
  print("[Stash] Cleared all pending operations.")
end

-- Utility: Check pending operations status
function getPendingStashStatus()
  if storage.lastOperationType == "withdraw" and storage.pendingWithdraws then
    local count = 0
    for _ in pairs(storage.pendingWithdraws) do count = count + 1 end
    print("[Stash] Pending withdraws: " .. count .. " items remaining")
    return "withdraw", count
  elseif storage.lastOperationType == "deposit" and storage.pendingDeposits then
    local count = 0
    for _ in pairs(storage.pendingDeposits) do count = count + 1 end
    print("[Stash] Pending deposits: " .. count .. " items remaining")
    return "deposit", count
  else
    print("[Stash] No pending operations.")
    return nil, 0
  end
end