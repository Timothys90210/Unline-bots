if AutoPartyLoaded then return end
AutoPartyLoaded = true
...
if rootWidget then
    ...
    autopartyui.ptShare:setOn(player:isPartySharedExperienceActive())
    autopartyui.ptShare.onClick = function(widget)
        local newState = not widget:isOn()
        widget:setOn(newState)
        g_game.partyShareExperience(newState)
    end
